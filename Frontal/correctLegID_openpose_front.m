function correctLegID_openpose_front(videoInfo)
...
h_axes_timeSeries = subplot(3,3,[1 2 4 5 7 8]); grid on
h_plot_timeSeries = plot(h_axes_timeSeries,trackedFrames,corrected_data(trackedFrames,[3 6 12 15],1),'.-'); 
...
legend(h_axes_timeSeries,'RShoulder','LShoulder','RAnkle','LAnkle','location','northwest'); 
...
h_axes_image = subplot(3,3,[3 6]);hold on
showImage

uicontrol(correctLegID_fig,'style','pushbutton','String','Switch left-right ID of legs',...
   'units','normalized','Position',[0.65 0.225 0.2 0.05],'Callback',@switchID); % pushbutton to switch
   
uicontrol(correctLegID_fig,'style','pushbutton','String','Delete left ID','foregroundcolor','b','backgroundcolor','w',...
   'units','normalized','Position',[0.65 0.15 0.1 0.05],'Callback',@clearLeft); % pushbutton to delete left leg ID

uicontrol(correctLegID_fig,'style','pushbutton','String','Restore left ID','foregroundcolor','b','backgroundcolor','w',...
   'units','normalized','Position',[0.65 0.1 0.1 0.05],'Callback',@restoreLeft); % pushbutton to restore left leg ID
...
uicontrol(correctLegID_fig,'style','pushbutton','String','Delete right ID','foregroundcolor','c','backgroundcolor','w',...
   'units','normalized','Position',[0.8 0.15 0.1 0.05],'Callback',@clearRight); % pushbutton to delete right leg ID

uicontrol(correctLegID_fig,'style','pushbutton','String','Restore right ID','foregroundcolor','c','backgroundcolor','w',...
   'units','normalized','Position',[0.8 0.1 0.1 0.05],'Callback',@restoreRight); % pushbutton to restore right leg ID
...
uicontrol(correctLegID_fig,'style','pushbutton','String','Restore entire data-series',...
   'units','normalized','Position',[0.65 0.025 0.2 0.05],'Callback',@restoreData); % pushbutton to restore entire data-series

uicontrol(correctLegID_fig,'style','pushbutton','String','Save',...
   'units','normalized','Position',[0.01 0.025 0.06 0.05],'Callback',@saveData); % pushbutton to save file
...
h_slider = uicontrol(correctLegID_fig,'style','slider','Min',startTrackFrame,'Max',endTrackFrame,'SliderStep',[1/noFrames_inspection 10/noFrames_inspection],'Value',frame,...
   'units','normalized','Position',[0.12 0.05 0.45 0.05],'Callback',@currentFrame_slider); % choose frame
% h_slider = uicontrol(correctLegID_fig,'style','slider','Min',startTrackFrame,'Max',endTrackFrame,'SliderStep',[1/noFrames_inspection 10/noFrames_inspection],'Value',frame,...
...
if event_obj.Target.Parent == h_axes_timeSeries 
...
global frame corrected_data frames frameInfo h_plot_timeSeries trackedFrames h_plot_ankleAngle h_image ...
    bilateral_temp_fields cursor_obj
...
temp = corrected_data(frame,[3:5 10:12 16 18 23:25],:);
corrected_data(frame,[3:5 10:12 16 18 23:25],:) = corrected_data(frame,[6:8 13:15 17 19 20:22],:);
corrected_data(frame,[6:8 13:15 17 19 20:22],:) = temp;
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
...
corrected_data(frame,[6:8 13:15 17 19 20:22],:) = nan;
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
...
corrected_data(frame,[6:8 13:15 17 19 20:22],:) = personTracked_data(frame,[6:8 13:15 17 19 20:22],:);
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
...
corrected_data(frame,[3:5 10:12 16 18 23:25],:) = nan;
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
...
corrected_data(frame,[3:5 10:12 16 18 23:25],:) = personTracked_data(frame,[3:5 10:12 16 18 23:25],:);
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
...
corrected_data(:,[3:8 10:25],:) = personTracked_data(:,[3:8 10:25],:);
h_plot_timeSeries(1).YData = corrected_data(trackedFrames,3,1);
h_plot_timeSeries(2).YData = corrected_data(trackedFrames,6,1);
h_plot_timeSeries(3).YData = corrected_data(trackedFrames,12,1);
h_plot_timeSeries(4).YData = corrected_data(trackedFrames,15,1);
