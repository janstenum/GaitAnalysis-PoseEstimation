function [videoInfo] = process_openpose_trackPerson_addition_sag()
...
% videoInfo.json_path = fullfile(videoInfo.vid_openpose_path,'JSON Files');
...
% end; clearvars i
[file videoInfo.json_path] = uigetfile({'*.JSON'},'Pick all JSON files','MultiSelect','on');
%%
...
for j = 1:data_openpose.noFiles
    if ~isnan(personTrack.trackPerson_inspect.inspectionTrack_personID(j))
        data_openpose.pose.data_personTracked_raw(j,:,:) = squeeze(data_openpose.pose.data_raw(personTrack.trackPerson_inspect.inspectionTrack_personID(j),j,:,:));
    end
end 
...
data_openpose.pose.data_personTracked_raw = squeeze(data_openpose.pose.data_raw(1,:,:,:));
...
opts.Default = 'Yes';
data_openpose.direction = questdlg([videoInfo.vid_openpose_name ': is the person walking right to left (\leftarrow)'],'Walking direction',opts);

if strcmp(data_openpose.direction,'Yes')
...
data_openpose.pose.data_personTracked_raw_reOriented(:,:,1) = data_openpose.pose.data_personTracked_raw(:,:,1);
data_openpose.pose.data_raw_reOriented(:,:,1) = data_openpose.pose.data_raw(:,:,1);
end
